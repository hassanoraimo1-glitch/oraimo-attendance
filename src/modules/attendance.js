...
async function loadEmpData() {
  ...
  try {
    ...
    // تحديث قائمة مبيعات اليوم فقط لو المستخدم فاتح تاب المبيعات حالياً،
    // عشان نتجنب race condition يمسح الداتا لو الاستدعاء فشل في الخلفية
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    err => {
      ...
      if (err.code === 1) {
        ...
        notify(ar ? '⚠️ تعذر الموقع، المتابعة بدونه' : '⚠️ Location failed, continuing without it', 'info');
      }
      openCamera();
    },
