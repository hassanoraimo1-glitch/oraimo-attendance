...
// ── CHECK-OUT GUARD: verify display photo uploaded today ──
async function _checkDisplayPhotoUploaded() {
  try {
    const today = todayStr();
    const records = await dbGet(
      'display_photos',
      `?employee_id=eq.${currentUser.id}&photo_date=eq.${today}&select=id&limit=1`
    ).catch(() => []);
    return records && records.length > 0;
  } catch (e) {
    console.warn('[_checkDisplayPhotoUploaded]', e);
    // If we can't verify, allow checkout (fail open, not fail closed)
    return true;
  }
}

function _showDisplayRequiredModal() {
  const ar = currentLang === 'ar';
  // Remove any existing instance first
  const existing = document.getElementById('display-required-modal-overlay');
  if (existing) existing.remove();

  const overlay = document.createElement('div');
  overlay.id = 'display-required-modal-overlay';
  overlay.style.cssText =
    'position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:8000;' +
    'display:flex;align-items:flex-end;backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px)';

  overlay.innerHTML = `
    <div style="
      background:linear-gradient(160deg,#15151c,#0a0a10);
    ...
    ">
      ...
      <div style="text-align:center;margin-bottom:16px">
        <div style="
          ...
          background:rgba(255,214,0,.12);border:2px solid rgba(255,214,0,.35);
          display:inline-flex;align-items:center;justify-content:center;font-size:34px
        ">🖼️</div>
      </div>

      <!-- Title -->
      <div style="font-size:17px;font-weight:900;text-align:center;margin-bottom:8px;color:var(--yellow)">
        ${ar ? '⚠️ لازم ترفع صورة الديسبلاي أولاً' : '⚠️ Display Photo Required'}
      </div>

      <!-- Body -->
      <div style="font-size:13px;color:var(--muted);text-align:center;margin-bottom:24px;line-height:1.6">
        ${ar
          ? 'مش قادر تسجل الخروج قبل ما ترفع صورة ديسبلاي اليوم.\nاضغط على الزر عشان ترفع الصورة وترجع تسجل خروجك.'
          : "You can't check out before uploading today's display photo.\nTap below to upload it, then come back to check out."
        }
      </div>

      <!-- Go upload button -->
      <button
        id="display-required-go-btn"
        style="
          width:100%;padding:15px;
          background:linear-gradient(135deg,#FFD600,#e6b800);
          border:none;border-radius:16px;
          color:#000;font-family:Cairo,sans-serif;font-size:15px;font-weight:900;
          cursor:pointer;margin-bottom:10px;
          box-shadow:0 4px 20px rgba(255,214,0,.35)
        "
      >
        🖼️ ${ar ? 'ارفع صورة الديسبلاي الآن' : 'Upload Display Photo Now'}
      </button>

      <!-- Cancel button -->
      <button
        id="display-required-cancel-btn"
        style="
  ...
  // Go to display tab
  document.getElementById('display-required-go-btn').addEventListener('click', () => {
    overlay.remove();
    // Navigate to the display tab
    const displayNavEl = document.getElementById('emp-display-nav');
    if (displayNavEl && typeof empTab === 'function') {
      empTab('display', displayNavEl);
      // Highlight the upload zone briefly so user notices it
      setTimeout(() => {
        const zone = document.getElementById('display-upload-zone');
        if (zone) {
          zone.style.transition = 'box-shadow .3s';
          zone.style.boxShadow = '0 0 0 3px var(--yellow)';
          setTimeout(() => { zone.style.boxShadow = ''; }, 2000);
        }
      }, 400);
    }
  });
  ...
  // Cancel — just close
  document.getElementById('display-required-cancel-btn').addEventListener('click', () => {
    overlay.remove();
  });
...
async function handleAttendClick() {
  ...
  // ── CHECKOUT GUARD: must have display photo for today ──
  if (attendMode === 'out') {
    btn.style.pointerEvents = 'none';
    const hasPhoto = await _checkDisplayPhotoUploaded();
    btn.style.pointerEvents = '';
    if (!hasPhoto) {
      _showDisplayRequiredModal();
      return;
    }
  }
