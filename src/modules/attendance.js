...
// ═══════════════════════════════════════════════════════════
// continueAttendProcess
// ═══════════════════════════════════════════════════════════
function continueAttendProcess() {
  const btn = document.getElementById('attend-btn');
  const ar = currentLang === 'ar';
...
function handleAttendClick() {
  ...
  // Prevent check-out without display photo
  if (attendMode === 'out') {
    checkTodayDisplay().then(hasDisplay => {
      if (!hasDisplay) {
        notify('❌ You must upload display photo before check out', 'error');
  ...
  try {
    ...
    if (attendMode === 'in') {
      if (todayAtt && todayAtt.length > 0) {
        notify('Already checked in', 'info');
        return;
      }
